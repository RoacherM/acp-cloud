...
- [ ] **Step 1: Write test that proves session/close is called**

The mock agent writes to stderr when `unstable_closeSession` is called. We can spy on the agent process stderr to verify the call was made. A simpler approach: verify that `close()` completes successfully when the agent supports `session/close` (the capability check + `unstable_closeSession` call path), and separately test that close still works when the capability is absent.

Add to `tests/session-controller.test.ts`:

```typescript
  it('close() completes successfully with session/close capable agent', async () => {
    ...
    // Verify mock agent declared session.close capability
...
Expected: FAIL — `agentCapabilities` does not exist on current `AgentHandle` (this depends on Task 2 being done first). If Task 2 is already done, this test will pass with existing close() behavior but will verify the capability check path once Step 3 is implemented.
...
```typescript
  it('shutdown closes all sessions without triggering crash handler', async () => {
    ...
    // Should see user_closed, NOT agent_crashed
    const statusChanges = events.filter(e => e.type === 'session_status_changed') as any[];
    const reasons = statusChanges.map((e: any) => e.reason);
    expect(reasons).toContain('user_closed');
...
Expected: May pass due to the `status === 'terminated'` guard in `handleCrash`, but the fix is still needed — relying on that guard is fragile. The test locks the correct behavior regardless.
...
describe('createServer', () => {
  ...
  describe('routes', () => {
    ...
    it('GET /sessions/:id/events returns SSE stream with events', async () => {
      runtime = makeRuntime();
      const app = createServer(runtime);
