...
it('spawn returns handle with agentCapabilities', async () => {
  ...
  expect(handle.agentCapabilities).toEqual({});
  pool.kill(handle);
});
```
...
Add the `closeSession` handler to the agent object (after the `cancel` handler):
...
- [ ] **Step 1: Write test for session/close before kill**

Add to `tests/session-controller.test.ts`:

```typescript
  it('close() sends ACP session/close when agent supports it', async () => {
    ...
    // Mock agent declares sessionCapabilities.close
    // close() should send session/close before killing
...
- [ ] **Step 2: Run test to confirm it passes (existing behavior still works)**

```bash
npx vitest run tests/session-controller.test.ts
```

Expected: PASS (close already works, we're adding session/close as a precursor).
...
Expected: May pass due to timing (the `handleCrash` guard catches it), but the fix is still needed for correctness.
...
function classifyError(err: unknown): { status: number; message: string } {
  ...
  if (/max active sessions|max agent processes/i.test(message)) {
...
export function createServer(runtime: CloudRuntime, opts?: ServerOptions): Hono {
  ...
  app.post(`${base}/sessions/:id/prompt`, async (c) => {
    const body = parseBody(PromptSchema, await c.req.json());
  ...
  app.post(`${base}/sessions/:id/permissions/:reqId/respond`, async (c) => {
    const body = parseBody(PermissionRespondSchema, await c.req.json());
...
```bash
timeout 5 node --import tsx examples/server.ts 2>&1 || true
```

Expected: Prints startup message, exits after 5s timeout.
