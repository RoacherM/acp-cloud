...
```bash
API_KEY= node --import tsx examples/server.ts &
SERVER_PID=$!
sleep 2
...
Expected: Server starts with auth disabled (`API_KEY=` empty), curl returns JSON with agents list, then server is killed.
